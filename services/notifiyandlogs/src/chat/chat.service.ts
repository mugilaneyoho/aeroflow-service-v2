...
export class ChatService {
  ...
  async getConversationChats(conversationId: string) {
    ...
    const memberMap = new Map(
      members.map((m) => [m.userId, { userName: m.userName, role: m.role }]),
    );
    ...
    const results = await Promise.all(
      conversations.map(async (conv) => {
        const unreadCount = await this.messageRepo
          ...
          .leftJoin('message.reads', 'reads', 'reads.userId = :userId', {
            userId,
          })
    ...
    if (dto.members && dto.members.length > 0) {
      await this.memberRepo.save(
        dto.members.map((member: any) => ({
          ...
          role:
            typeof member === 'object' && member.role ? member.role : 'STAFF',
          userName:
            typeof member === 'object' && (member.name || member.userName)
  ...
  async getConversationMembers(conversationId: string) {
    return this.memberRepo.find({
      where: { conversationId: conversationId, deletedAt: IsNull() },
    });
  }
